...
function VariantsValues() {
    const [variants, setVarians] = useState([]);
    const [variant_Value, setVariant_Value] = useState([]);
    ...
    const [viewVariantName, setViewVariantName] = useState('');
    ...
    // Read variants-value from Api

    useEffect(() => {
        fetch(`${window.baseUrl}variants-value`)
            .then((res) => {
                if (res.status === 200) {
                    return res.json()
                }
    ...
    const handleSubmit = (e) => {
        ...
        fetch(`${window.baseUrl}variants-value`, {
            ...
            .then(data => setVariant_Value([...variant_Value, data.data])
            )
            .catch((error) => {
                console.log(error);
            })
...
const ViewVariant_Value= (id) => {
    console.log(id)
    fetch(`${window.baseUrl}variants-value/${id}`)
      .then((res) => {
        if (res.status === 200) {
          return res.json()
        }
      ...
      .then(data => {
        setViewVariantName(data.data.variant_name)
    ...
    const editcategory = (id) => {
        fetch(`${window.baseUrl}variants-value/${id}`)
            ...
            .then(data => {
        console.log(data.data)
                setVariantId(data.data.variant_id)
                setEditId(data.data.variant_values_id)
    ...
    const handleEditSubmit = (e) => {
        ...
        fetch(`${window.baseUrl}variants-value/${editId}`, {
            method: 'POST',
            body: JSON.stringify({
                variant_id: variantId,
                name: editName,
            ...
            .then(data => {
                setVariant_Value([...variant_Value.filter((variant_v) => variant_v.variant_id !== editId), data.data])
    ...
    const deletecategory = (id) => {
        setVariant_Value(variant_Value.filter((sub_categorie) => sub_categorie.sub_category_id !== id));
        fetch(`${window.baseUrl}variants-value/${id}`, {
            method: 'DELETE'
        })
    }
    ...
    const pageCount = Math.ceil(variant_Value.length / itemsPerPage)
    const changePage = ({ selected }) => {
        setPageNumber(selected)
    }

    const displayitems = variant_Value ? variant_Value.slice(PagesVisited, PagesVisited + itemsPerPage)
        .map((item,index) => {
            console.log(item)
            return (<div className="item-management-variant_Value items_contents" key={index}>
                    ...
                    <div className="item_content">
                        <div className='item_element'>
                            <div className="item_element_buttons">
                                <button className='item_element_button' onClick={() => ViewVariant_Value(item.variant_id)}><img src={Visible} alt="Visible" /></button>
        ...
        <>
                ...
                <div className="body_header">
                    <nav>
                        <ol className="breadcrumb">
                            ...
                            <li className="breadcrumb-item active">variants-value</li>
                        </ol>
                    </nav>
            ...
            <Modal show={show} setShow={setShow}>
                <div className='input_group'>
                    <label htmlFor="" className='input_field_label'>variants</label>
                    <select className='input_field' type="text" defaultValue={'DEFAULT'} onChange={(e) => setVariantId(e.target.value)}>
                        <option value="DEFAULT" disabled hidden>select Variant</option>
                        {variants.map((variant,index)=>{
                            console.log(variant.variant_id)
                            return (
                                <option value={variant.variant_id} key={index}>{variant.name}</option>
                            )
                        })}
                ...
                <div className='input_group'>
                    <label htmlFor="" className='input_field_label'>value</label>
            ...
            <Modal show={EditShow} setShow={setEditShow} title={"Edit Category"}>
                <div className='input_group'>
                    <label htmlFor="" className='input_field_label'>Variants</label>
                    <select className='input_field' type="text" defaultValue={'DEFAULT'} onChange={(e) => setVarians(e.target.value)}>
                    <option value="DEFAULT" disabled hidden>select Variant</option>
                    {variants.map((variant,index)=>{
                            return (
                                <option value={variant.variant_id} key={index}>{variant.name}</option>
